...
class GetLabelsService(private val githubClientService: GithubClientService) {
    fun getLabelsForOrg(org: String): List<String> = githubClientService.fetchReposForOrg(org)
        .flatMapParallel { getLabelsForRepo(org, it.name) }

    fun getLabelsForRepo(org: String, repo: String) =
        githubClientService.fetchLabelsForRepo(org, repo)
            .labelNames()

    fun getIssuesForMarkedForContribution(org: String): List<IssueSummary> = githubClientService.fetchReposForOrg(org)
